...
import { getAtlasPoints, type AtlasPoint } from '../../services/atlas'
    ...
    return (
        ...
        >
            ...
            <Globe
                ...
                showGraticules
                graticulesColor={colors.grid}
                pointsData={points}
                pointAltitude={0.02}
                pointRadius={0.8}
                pointColor={() => colors.point}
                pointLabel={(d:AtlasPoint)=>`${t(`region.name.${d.region}` as any) || d.name}`}
                onPointClick={(d:AtlasPoint)=> nav(`/atlas/${d.region}`)}
            />
        </div>
    )
}
