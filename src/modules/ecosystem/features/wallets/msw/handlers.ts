...
export const walletHandlers = [
    ...
    http.post('/api/login', async ({ request }) => {
        ...
        if (!identifier || !password) return HttpResponse.text('账户或密码错误', { status: 401 })
    ...
    http.get('/api/dashboard', async ({ request }) => {
        ...
        console.log('[MSW] /api/dashboard → mixed mode ON')

        // ——“混合模式”：真实 + 兜底 —— //
        type RealResp = {
            code?: string | number
            message?: string
            data?: { totalDollar?: string | number; dataList?: RealAsset[] }
        }

        // 地址从环境变量取；默认为你之前那串
        const ADDRESS =
            (import.meta.env.VITE_WALLET_ADDRESS as string | undefined) ??
            'TSdwrV1V5UbR9iNss6d5mGZN5pa7BpIB2A' // ← 修正了之前的笔误

        let real: RealResp = { data: { totalDollar: '0', dataList: [] } }
        try {
            if (!REAL_BASE) throw new Error('VITE_API_PROXY_TARGET not set')

            // ② 如果 REAL_BASE 是绝对地址或代理前缀，都统一拼接：
            //    - 绝对地址示例: http://192.168.99.88:8080
            //    - 代理前缀示例: /realapi   （推荐，通过 Vite 代理）
            const endpoint = `${REAL_BASE.replace(/\/$/, '')}/account/get`
            ...
            console.log('[MSW] real endpoint =', endpoint, 'status =', resp.status)
            if (!resp.ok) throw new Error(await resp.text())
            real = (await resp.json()) as RealResp
        } catch (e) {
            console.warn('[MSW] /api/dashboard real POST failed:', e)
        }

        const totalUSD: number = Number(real?.data?.totalDollar ?? 0)
        const list: RealAsset[] = Array.isArray(real?.data?.dataList) ? real.data!.dataList! : []

        // 把真实 dataList → 前端 mainAssets
        const mainAssets: MainAsset[] = list.map((it: RealAsset) => ({
            icon:   (it.tokenAbbr ?? '').toString().toUpperCase(),
            name:   (it.tokenAbbr ?? '').toString().toUpperCase(),
            symbol: (it.tokenAbbr ?? '').toString().toUpperCase(),
            amount: Number(it.accountBalance ?? 0),
            usd:    Number(it.dollarAccountBalance ?? 0),
            changePct: 0,
        }))

        const distribution = computeDistributionFrom(mainAssets.map((x: MainAsset) => ({ name: x.name, usd: x.usd })))
        const market = mockMarket()
        const activities = mockActivities()

        // ③ 注意：overview.totalUSD 用真实的 totalUSD（数字），不会再出现 127K
        return HttpResponse.json({
