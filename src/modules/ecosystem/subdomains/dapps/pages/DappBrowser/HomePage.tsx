        ...
        <div className="container vertical">
            ...
            <div className="grid cols-5 gap">
                {['DeFi','GameFi','NFTs','Social','Tools'].map(c=>(
                    <button key={c} className="category-card">
            ...
            <SectionHeader
                title={t('home.rankings')} sub={t('home.rankingsSub')}
                right={
                    <div className="toolbar">
                        <input className="input" placeholder={t('common.searchDapp') as string}/>
                        <button className="btn ghost">{t('common.search')}</button>
                        <button className="btn ghost">{t('common.sort')}</button>
                        <button className="btn ghost">{t('common.timeRange')}</button>
                    </div>
                }
            />
            ...
            <div className="grid cols-2 gap">
                <div className="panel tall">
                    ...
                    <div className="browse-grid">
                        {[['DeFi',234],['GameFi',156],['NFTs',89],['Social',67],['Tools',45],['Analytics',23]]
                            .map(([k,c])=>(
                                <button key={k as string} className="pill">
                                    <span className="dot"/><span className="k">{k as string}</span><span className="c">{c as number}</span>
                                </button>
                            ))}
                    </div>
                </div>
                ...
                <div className="panel tall">
                    <div className="panel-head">{t('home.curatedCollections')}</div>
                    <div className="curated-list">
                        {[{title:t('home.curated.highYieldDeFi'), count:8, chips:['JS','SUN','JL','+5']},
                            ...
                            .map((s,idx)=>(
                                <div key={idx} className="curated-item">
                                    <div>
                                        <div className="ci-title">{s.title}</div>
                                        <div className="ci-chips">
                                            {s.chips.map((c,i)=>(<span key={i} className="chip">{c}</span>))}
                                        </div>
                                    </div>
                                    <button className="btn ghost sm">{s.count}{t('home.curated.dapps')}</button>
                                </div>
                            ))}
                    </div>
                </div>
            ...
            <div className="grid cols-4 gap">
                {featured.map(d=> <DappCard key={d.id} dapp={d}/>)}
            </div>
        </div>
    );
}
