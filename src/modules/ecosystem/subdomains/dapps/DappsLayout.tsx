...
export default function DappsLayout() {
  ...
  const [collapsed, setCollapsed] = useState(false); // 侧栏是否导轨折叠

  return (
    <div className="dapps-shell">
      <div className="dapps-main">
        <aside className={`dapps-sider ${collapsed ? 'rail' : ''}`}>
          {/* 折叠按钮（放在侧栏里，行为与 SrLayout 一致） */}
          <button
            className="dapps-sider-toggle-btn"
          ...
          <div className="dapps-sider-title">{t('nav.title')}</div>
          ...
          <div className="dapps-sider-sub">{t('nav.extra')}</div>
        </aside>

        <main className="dapps-content">
          <div className="dapps-content-inner">
            <Outlet />
          </div>
        </main>
      </div>

      <footer className="dapps-footer">
        <div className="dapps-footer-inner">
...
function SiderItem({
  ...
  return (
    <NavLink
      ...
      className={({ isActive }) => `dapps-sider-item ${isActive ? 'active' : ''}`}
    >
      <span className="dapps-ic">{icon}</span>
      <span className="dapps-txt">{children}</span>
    </NavLink>
  );
}

/* === inline icons：stroke="currentColor" 跟随项状态变色 === */
function IconHome() {
  return (
